    ...
    def post(self, request, format=None):
        ...
        if code != None:
            ...
            if len(room_result) > 0:
                room = room_result[0]
                # Make a note on backend that this user is in this room. Temp storage object.
                # So user can be returned to the same room if they leave.
...
class UserInRoom(APIView):
        def get(self, request, format=None):
            # first check for active session, if not, create one.
            if not self.request.session.exists(self.request.session.session_key):
                self.request.session.create()
...
class LeaveRoom(APIView):
    def post(self, request, format=None):
        if "room_code" in self.request.session:
            # removes code when leaving room
            self.request.session.pop("room_code")
            # gives the room code to check if user was the host
